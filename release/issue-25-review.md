...
Acceptance: at least 2,300 prose words per chapter/language (excluding references,
footnotes and executable blocks), with the measured count reported and no hard
maximum; paired required sections/workflows; reproducible
...
| Automated assurance | generic paired/build checks | specific SQL replacement/temporal tests, singleton projection test, independent graph and affine benchmarks, exact source inventory, provenance hashes, standalone archive reproduction and paired executable-block/reference checks |
...
assertion 1 -- zero-or-one assertion.supersedes   (append-only correction)
...
The SQLite fixture has 13 stored assertions and 12 current assertions. A replacement
must retain the same subject, predicate, context, source, validity interval and
interval kind, and one assertion can have at most one direct successor. Regression
tests reject a narrower replacement and a second branch while accepting one valid
replacement. Other constraints reject orphan subjects/sources/objects, invalid
value/object combinations, reversed intervals, updates and deletion. The Python
importer additionally rejects non-canonical dates, incomplete or mis-anchored source
inventories and duplicate or orphan participation. These are consistency checks,
not truth tests.
...
The authentic map is Koch's *Ljubljana* (1910), from dLib.si; the item-level rights
block links its “javna domena” badge to the provider's Public Domain Mark statement.
`source/dlib-ljubljana-1910.json` records the URN, canonical and download URLs,
provider, bibliographic fields, dimensions, digest, rights badge/link/target and
2026-09-07 access date. The unchanged 5747 × 7287 JPEG is pinned by
...
| Name/boundary time | name changes in 1917 inside the 1910 boundary version | true interval intersection yields 1917–1920 | start-only temporal join is rejected |
...
| Singleton document | retained person–document edge | no projected pair or fractional divisor | bipartite evidence is not silently lost |
...
| Page | Words |
|---|---:|
| English databases | 2,719 |
| Slovene databases | 2,634 |
| English GIS | 2,655 |
| Slovene GIS | 2,575 |
| English networks | 2,710 |
| Slovene networks | 2,593 |
...
46 members and matches its committed digest byte-for-byte. CI now installs the
pinned GIS requirement, runs the coordinate check and rebuilds this packet before
testing for an uncommitted generated diff.

The earlier rendered inspection covered all six chapter pages and all ten workflow pages in
...
## Slovene editorial pass and review state

On 2026-09-07 an agent-assisted continuity pass covered the three Slovene chapters,
five workflows and four packet documents. It revised every expression identified
in the PR #33 editorial review and the adjacent prose, and standardized the chapter
reference as *Modeli, dokazno gradivo in interpretacija*. This was not a competent
human language approval. The machine-assisted notices therefore remain visible.

The checker no longer permanently requires draft wording. It accepts the current
machine-assisted state with a visible notice, or a future `human-reviewed` state
only when `translation_reviewed_by`, `translation_reviewed_on` and
`translation_review_scope` are recorded in front matter.
...
- manual QGIS 3.40-or-compatible execution, visual comparison and re-selection of
  ...
  no approved georeferenced raster is claimed. The exact fields are prepared, but
  intentionally unchecked, in `release/issue-25-qgis-review-checklist.md`;
