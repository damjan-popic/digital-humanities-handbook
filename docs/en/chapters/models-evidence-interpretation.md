...
Record whether a category comes from the historical source, the research design or annotated examples. A nineteenth-century catalogue label and a normalized modern genre are two pieces of evidence, not synonyms. Preserve source wording where feasible, allow uncertain or mixed cases when the concept requires them, and inspect “other” for institutional exclusions.
...
A confidence interval can express sampling or model uncertainty under warranted assumptions. It cannot repair a missing archive, decide whether irony counts as fear, or represent every interpretive disagreement. Published code reveals a procedure, not the adequacy of its concept.
...
| **Manual expressed/attributed annotation** | Unit: clause plus speaker, possible experiencer and target. Categories: speaker expresses fear/hope; speaker attributes it to another actor; quoted expression; mixed/uncertain; absent. | Output: annotated passages, category proportions and coder disagreement. Validate through pilot coding, independent double annotation, adjudication logs, counterexamples and reading full speech turns. | Supports an account of how debate allocates emotion rhetorically among actors. Cannot reveal private states or cheaply characterize every speech; agreement does not by itself prove conceptual validity. | Gains role, target, implicit phrasing and contextual nuance; loses scale and introduces documented interpretive labour. |
| **Supervised classifier with embeddings** | Unit: sentence or short speech turn. Categories inherit the manual scheme; an embedding represents contextual similarity, and the classifier estimates labels or probabilities. | Output: held-out predictions and aggregate estimates. Split by session and period; report class-specific precision/recall, calibration and confusion; test historical subperiods; read errors and nearest neighbours; compare with lexicon and majority baselines. | Supports an estimated distribution of the annotation scheme if performance and domain stability are adequate. Cannot turn labels into natural kinds, recover excluded archives or establish the effect on audiences. | Gains scalable context sensitivity and retrieval of semantically related phrasing; loses direct rule transparency, compresses language into learned dimensions and risks period or source shift. |
...
A responsible interpretation might conclude: *within the licensed corpus of sessions, explicit and attributed fear vocabulary became more frequent after a specified date, while independently coded expressions by speakers did not rise at the same rate*. Historical explanation would then require close reading and contextual sources about policy events, parliamentary rhetoric and corpus composition. None of the tables alone explains the change.
...
An output becomes evidence only when provenance, validity, comparison and claim fit have been established. Evidence constrains but does not automate interpretation. Close, distant, error and outlier reading, followed by contextual return, make scale an iterative movement through sources and models. Missing archives, ambiguous categories, measurement error, model instability, visual rhetoric and interpretive alternatives are different uncertainties and require different responses. The aim is not a frictionless pipeline, but an inspectable argument in which another scholar can see what was taken, transformed, tested, learned and left unresolved.
